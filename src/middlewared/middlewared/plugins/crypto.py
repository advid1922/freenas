...
class CryptoKeyService(Service):
    ...
    def generate_crl(self, ca, certs, next_update=1):
        ...
        # compromised.
        #
        # `ca` is either root ca which has been compromised as well or the parent of a ca
        # which has been revoked. `certs` is a list of all certs ca inclusive which are to
        # included in the CRL ( if root ca is compromised, it will be in `certs` as well ).
        # TODO: Verify how openvpn handles root ca revocation if it gets included in CRL
        # does it fail to start etc ?
        ...
        crl_builder = x509.CertificateRevocationListBuilder().issuer_name(x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, ca['common'])
        ])).last_update(
    ...
    async def do_update(self, job, id, data):
        ...
        if any(new[k] != old[k] for k in ('name', 'revoked')):
            ...
            if old['revoked'] != new['revoked'] and new['revoked']:
                revoked = {'revoked': True, 'revoked_date': datetime.datetime.utcnow()}
            else:
                revoked = {}
